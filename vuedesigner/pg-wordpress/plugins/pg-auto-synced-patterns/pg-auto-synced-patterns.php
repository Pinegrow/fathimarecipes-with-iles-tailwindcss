 ...
 *   (for convenience for constrained roles)
 */
...
add_action('admin_menu', function () {
  ...
  // Only allow these core/admin pages
  $allowed = [
    'index.php',                   // Dashboard
    'edit.php',                    // Posts
    'upload.php',                  // Media
  ...
  // Remove any top-level menu not in allowed
  foreach ($menu as $index => $item) {
    $slug = $item[2] ?? '';
    $label = wp_strip_all_tags($item[0] ?? '');

    // Cloudinary removal by label (extra safe)
    if (stripos($label, 'cloudinary') !== false) {
